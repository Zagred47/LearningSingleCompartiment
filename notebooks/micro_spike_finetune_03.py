...
print("parameters:", count_trainable_parameters(finetune_model))
...
def validation_metrics(model):
    ...
    for offset in range(0, len(val_x), CFG.batch_trajectories):
        ...
        for start in range(0, inputs.shape[1], CFG.chunk_steps):
            ...
            value, terms = criterion(prediction, target)
...
LAST_CHECKPOINT = CHECKPOINTS / "gru_spike_finetune_last.pt"
BEST_CHECKPOINT = CHECKPOINTS / "gru_spike_finetune_best.pt"
...
for epoch in epoch_bar:
    ...
    for offset in range(0, len(sampled), CFG.batch_trajectories):
        ...
        with torch.no_grad():
            for start in range(0, context_t.shape[1], CFG.chunk_steps):
                _, hidden = finetune_model(context_t[:, start : start + CFG.chunk_steps], hidden)
                hidden = detach(hidden)
        optimizer.zero_grad(set_to_none=True)
        with torch.autocast("cuda", dtype=torch.float16, enabled=DEVICE.type == "cuda"):
            prediction, _ = finetune_model(window_t, hidden)
            loss, _ = criterion(prediction, target_t)
        scaler.scale(loss).backward()
        scaler.unscale_(optimizer)
        nn.utils.clip_grad_norm_(finetune_model.parameters(), CFG.gradient_clip)
        scaler.step(optimizer)
    ...
    epoch_bar.set_postfix(
        ...
        val_event=f"{validation['rare_event']:.3e}",
        scale=f"{scale:.2f}",
        eta=f"{eta / 60:.1f}m",
    )
...
predictions = {
    ...
    "gru_spike_finetune": predict(finetune_model),
}
rows = []
...
for name, prediction in predictions.items():
    ...
    row = {
        "model": name,
        "parameters": count_trainable_parameters(baseline_model),
...
axis.plot(time_ms, predictions["gru_spike_finetune"][trajectory, :steps, soma_index], label="GRU fine-tune", alpha=0.85)
...
(OUTPUT / "experiment.json").write_text(json.dumps({
    "contract": "GRU-MSE checkpoint -> conservative rare-event fine-tuning",
    "dataset": str(DATASET),
